...
rutils::slashcommand_group_t::slashcommand_group_t(std::unique_ptr<precondition_t>&& precondition, dpp::slashcommand&& command, dpp::snowflake guild_id, command_function_t function)
	: slashcommand_subgroupable_i(std::move(precondition))
...
(
	...
	dpp::parameter_registration_t&& parameters, std::unique_ptr<precondition_t>&& precondition,
...
)
{
	...
	command_sub_commands.emplace_back(std::move(precondition), sub_command_option, std::move(function));
...
(
	const std::string& name, const std::string& description, std::unique_ptr<precondition_t>&& precondition
)
{
	command.command.options.reserve(10);
	dpp::command_option& option = command.command.options.emplace_back(dpp::co_sub_command_group, name, description);

	return command_sub_groups.emplace_back(std::move(precondition), option);
}

rutils::slashcommand_sub_group_t::slashcommand_sub_group_t(std::unique_ptr<precondition_t>&& precondition, dpp::command_option& command_group)
	: slashcommand_subgroupable_i(std::move(precondition))
...
(
	const std::string& name, const std::string& description, dpp::parameter_registration_t&& parameters, std::unique_ptr<precondition_t>&& precondition, command_function_t function
)
{
...
)
{
	...
	return command_sub_groups.emplace_back(std::move(precondition), sub_command_group_option);
}
